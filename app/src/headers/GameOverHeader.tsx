...
export const GameOverHeader = () => {
  ...
  if (winWithThreeSameColor) {
    return <Trans i18nKey="header.end.same.color.player" values={{ player: winnerName }} />
  }

  if (winWithAllColor) {
    return <Trans i18nKey="header.end.all.colors.player" values={{ player: winnerName }} />
  }
  ...
  if (winnerCharacters > loserCharacters) {
    return <Trans i18nKey="header.end.characters.player" values={{ player: winnerName, winnerCharacters, loserCharacters }} />
  }

  return <Trans i18nKey="result.comp.tie.all" ns="common" />
}
